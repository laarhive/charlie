// src/devices/kinds/ld2410Radar/ld2410RadarDevice.js
...
export default class Ld2410RadarDevice extends BaseDevice {
  ...
  constructor({ logger, clock, domainBus, mainBus, device, protocolFactory }) {
    ...
    if (!this.#mainBus?.publish) {
      throw new Error('ld2410Radar requires main bus for system:hardware reporting')
    }

    this.#decoder = createLd2410StreamDecoder({
      includeRaw: false,
      maxBufferBytes: 8192,
      emitStats: false,
    ...
    this.#decoderOnError = (e) => {
      this.#logger?.error?.('ld2410_decode_error', {
        deviceId: this.getId(),
        code: e?.code,
        message: e?.message,
