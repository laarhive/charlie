// src/devices/kinds/ld2460Radar/ld2460RadarDevice.js
...
import { createLd2460StreamDecoder } from './ld2460Decode.js'
...
export default class Ld2460RadarDevice extends BaseDevice {
  #logger
  #clock
  #domainBus
  #mainBus
  ...
  constructor({ logger, clock, domainBus, mainBus, device, protocolFactory }) {
    ...
    if (!this.#mainBus?.publish) {
      throw new Error('ld2460Radar requires main bus for system:hardware reporting')
    }

    this.#decoder = createLd2460StreamDecoder({
    ...
    this.#decoderOnError = (e) => {
      this.#logger?.error?.('ld2460_decode_error', {
        deviceId: this.getId(),
        code: e?.code,
        message: e?.message,
  ...
  #publishFrame(frame) {
    ...
    this.#domainBus.publish({
      ...
      payload: {
        ...
        // No HW timestamp exists for LD2460. measTs is the decoder's frame completion time.
        measTs,
        frame,
      },
    })
